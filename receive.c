...
int control_recv(char* device) 
{
	...
	if((fd=_open(device, _O_RDWR|_O_BINARY|_O_NONBLOCK ))==-1){
        ...
        printf("[%s %d] Open %s succeed\n", __FUNCTION__, __LINE__, device);
    }
    
	count_read=0;
    //tcflush(fd, TCIOFLUSH);
    printf("[%s %d] Ready to receive file ...\n", __FUNCTION__, __LINE__);
	for(;;)
	{
		delay(10000000);
	...
	{		
        ...
        //printf("[%s %d]count_read %u\n", __FUNCTION__, __LINE__, count_read);
		if(count_read)
		{
			if(PacketStartFlagRecv)
			{	
				PacketStartFlagRecv=0;
				printf("[%s %d] 0x%x\n", __FUNCTION__, __LINE__, rxdata[0]);
				switch(rxdata[0])
				{
					case SOH:
						if(CAN_CNT)
		...
		if(!count_read) {//get EOT or unvilid packet head or CAN 
            printf("[%s %d] count_read %d, get EOT or unvilid packet head or CAN\n",
                __FUNCTION__, __LINE__, count_read);
			continue;
		}
        //printf("count_read %d PacketLen %d\n",count_read, PacketLen);
		if(count_read==PacketLen)
		{
#ifdef DEBUG			
			printf("get a packet completly!count_read:%d, PacketLen %d\n",
			        count_read, PacketLen);
#endif
			PacketStartFlagRecv=1;
			count_read=0;
